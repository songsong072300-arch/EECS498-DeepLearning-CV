...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        N = X.shape[0]
        x_load = X.reshape(N, -1)
        h_lay1 = x_load.mm(W1) + b1
        scores_lay1 = h_lay1*(h_lay1>0)
        scores = scores_lay1.mm(W2) + b2
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dscores = probs.clone()
        dscores[torch.arange(N), y] -= 1.0
        dscores /= N
        grads['W2'] = scores_lay1.t().mm(dscores) + 2 * reg * W2
        grads['b2'] = dscores.sum(dim=0)
        dh_relu = dscores.mm(W2.t())
        dh_lay1 = dh_relu * (h_lay1 > 0)
        grads['W1'] = x_load.t().mm(dh_lay1) + 2 * reg * W1
        grads['b1'] = dh_lay1.sum(dim=0)
        ###################################################################
        #                     END OF YOUR CODE                            #
        ###################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            ...
            b_shape = (dims[i + 1],)
            self.params[f'W{idx}'] = torch.randn(w_shape, dtype=dtype, device=device) * weight_scale
            self.params[f'b{idx}'] = torch.zeros(b_shape, dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        relu_masks={}
        N=X.shape[0]
        scores_tmp = X.reshape(N,-1)
        for i in range(1,self.num_layers):
            W = self.params[f'W{i}']
            b = self.params[f'b{i}']
            caches[i] = scores_tmp
            scores_tmp = scores_tmp.mm(W)+b
            mask = scores_tmp>0
            relu_masks[i] = mask
            scores_tmp = scores_tmp*mask
        last_idx = self.num_layers
        W_last = self.params[f'W{last_idx}']
        b_last = self.params[f'b{last_idx}']
        caches[last_idx] = scores_tmp
        scores = scores_tmp.mm(W_last) + b_last
        ...
        loss = torch.sum(-torch.log(p[torch.arange(N), y])) / N
        for i in range(1,self.num_layers+1):
            Wi = self.params[f'W{i}']
            loss += 0.5*self.reg*(Wi*Wi).sum()
        dscores = p.clone()
        dscores[torch.arange(N), y] -= 1.0
        dscores /= N
        W_last = self.params[f'W{last_idx}']
        x_last = caches[last_idx]
        grads[f'W{last_idx}'] = x_last.t().mm(dscores) + self.reg * W_last
        grads[f'b{last_idx}'] = dscores.sum(dim=0)
        dscores = dscores.mm(W_last.t())
        for i in range(self.num_layers - 1, 0, -1):
            dscores = dscores * relu_masks[i]
            W = self.params[f'W{i}']
            x_in = caches[i]
            grads[f'W{i}'] = x_in.t().mm(dscores) + self.reg * W
            grads[f'b{i}'] = dscores.sum(dim=0)
            dscores = dscores.mm(W.t())
        ###########################################################
        #                   END OF YOUR CODE                      #
        ###########################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            random = torch.rand(x.shape,dtype=x.dtype,device=x.device)
            mask = random < 1-p
            out = x*mask/(1-p)
            ##############################################################
            #                   END OF YOUR CODE                         #
