...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        padconfig = (pad,pad,pad,pad)
        x_pading = torch.nn.functional.pad(x,pad=padconfig,mode='constant',value=0.0)
            ...
            for f in range(F):
                for i in range(out_h):
                    for j in range(out_w):
                        ...
                        w_start = j * stride
                        w_end = w_start + WW
                        x_slice = x_pading[n, :, h_start:h_end, w_start:w_end]
    ...
    def backward(dout, cache):
        ...
        stride,pad = conv_param.get('stride'),conv_param.get('pad')
        pad_config = (pad,pad,pad,pad)
        x_pad = torch.nn.functional.pad(x, pad=pad_config, mode='constant', value=0.0)
            ...
            for f in range(F):
                for i in range(out_h):
                    for j in range(out_w):
                        ...
                        w_end = w_start + WW
                        da=dout[n,f,i,j]
                        dw[f]+=x_pad[n, :, h_start:h_end, w_start:w_end]*da
                        dx_pad[n, :, h_start:h_end, w_start:w_end]+=w[f]*da
        if pad>0:
    ...
    def forward(x, pool_param):
        ...
        for i in range(out_H):
            for j in range(out_W):
                ...
                w_start = j * stride
                w_end = w_start + width
                out[:,:,i,j] = x[:,:,h_start:h_end,w_start:w_end].flatten(start_dim=2).max(dim=2)[0]
    ...
    def backward(dout, cache):
        ...
        for i in range(out_H):
            for j in range(out_W):
                ...
                w_end = w_start + width
                da=dout[:,:,i,j]
                idx = x[:,:,h_start:h_end,w_start:w_end].flatten(start_dim=2).max(dim=2)[1]
                num=height*width
                mask=torch.nn.functional.one_hot(idx,num_classes=num).to(x.dtype)
                grad_flat = da.unsqueeze(-1) * mask
                grad_window = grad_flat.view(N, C, height, width)
                dx[:, :, h_start:h_end, w_start:w_end] += grad_window
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        self.params["W1"] = torch.randn(w1_shape, dtype=dtype, device=device) * weight_scale
        self.params["b1"] = torch.zeros(num_filters, dtype=dtype, device=device)
        conv_output_dim = num_filters * (H // 2) * (W // 2)
        w2_shape = (conv_output_dim, hidden_dim)
        self.params['W2'] = torch.randn(w2_shape, dtype=dtype, device=device) * weight_scale
        self.params['b2'] = torch.zeros(hidden_dim, dtype=dtype, device=device)
        w3_shape = (hidden_dim, num_classes)
        self.params['W3'] = torch.randn(w3_shape, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        first_out, CRP_cache = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        b, LR_cache = Linear_ReLU.forward(first_out, W2, b2)
        scores, L_cache = Linear.forward(b, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        reg_loss = self.reg * (torch.sum(W1 ** 2) + torch.sum(W2 ** 2) + torch.sum(W3 ** 2))
        loss += reg_loss
        db, dW3, db3 = Linear.backward(dscores, L_cache)
        dfirst_out, dW2, db2 = Linear_ReLU.backward(db, LR_cache)
        dX, dW1, db1 = Conv_ReLU_Pool.backward(dfirst_out, CRP_cache)
        dW3 += 2*self.reg*W3
        dW2 += 2*self.reg*W2
        dW1 += 2*self.reg*W1
    ...
    def __init__(self,
        ...
        for i, out_channels in enumerate(num_filters):
            ...
            if weight_scale == 'kaiming':
                fan_in = in_channels * 3 * 3
                scale = (2.0 / fan_in) ** 0.5
            else:
                scale = weight_scale
            w_shape = (out_channels, in_channels, 3, 3)
            self.params[f'W{layer_idx}'] = torch.randn(w_shape, dtype=dtype, device=device) * scale
            self.params[f'b{layer_idx}'] = torch.zeros(out_channels, dtype=dtype, device=device)

            if self.batchnorm:
                self.params[f'gamma{layer_idx}'] = torch.ones(out_channels, dtype=dtype, device=device)
                self.params[f'beta{layer_idx}'] = torch.zeros(out_channels, dtype=dtype, device=device)
            in_channels = out_channels
            if i in self.max_pools:
                H //= 2
                W //= 2
        last_idx = self.num_layers
        fc_in_dim = in_channels * H * W
        
        if weight_scale == 'kaiming':
            scale = (2.0 / fc_in_dim) ** 0.5
        else:
            scale = weight_scale

        self.params[f'W{last_idx}'] = torch.randn((fc_in_dim, num_classes), dtype=dtype, device=device) * scale
        self.params[f'b{last_idx}'] = torch.zeros(num_classes, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            if self.batchnorm:
                ...
                out, bn_cache = SpatialBatchNorm.forward(out,gamma, beta, bn_param)
            out, relu_cache =ReLU.forward(out)
            pool_cache = None
            if i in self.max_pools:
                out, pool_cache = FastMaxPool.forward(out, pool_param)
            caches[layer_idx] = (conv_cache, bn_cache, relu_cache, pool_cache)
        last_idx = self.num_layers
        W_last = self.params[f'W{last_idx}']
        b_last = self.params[f'b{last_idx}']
        
        scores, fc_cache = Linear.forward(out, W_last, b_last)
        caches[last_idx] = fc_cache
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        last_idx = self.num_layers
        fc_cache = caches[last_idx]
        dout, dW, db = Linear.backward(dscores, fc_cache)
        grads[f'W{last_idx}'] = dW + 2 * self.reg * self.params[f'W{last_idx}']
        grads[f'b{last_idx}'] = db
        for i in range(self.num_layers-1,0,-1):
            conv_cache, bn_cache, relu_cache, pool_cache =caches[i]
            if pool_cache is not None:
                dout = FastMaxPool.backward(dout, pool_cache)
            dout = ReLU.backward(dout, relu_cache)
            if self.batchnorm:
                dout, dgamma, dbeta = SpatialBatchNorm.backward(dout, bn_cache)
                grads[f'gamma{i}'] = dgamma
                grads[f'beta{i}'] = dbeta
            dout, dW, db = FastConv.backward(dout, conv_cache)
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        scale = (gain / Din).sqrt()
        size = (Din, Dout)
        weight = torch.randn(size, dtype=dtype, device=device)*scale
        ###################################################################
        #                            END OF YOUR CODE                     #
        ###################################################################
    else:
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            cache = (x, gamma, x_hat, mean, var, eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == 'test':
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x, gamma, x_hat, mean, var, eps = cache
        N = x.shape[0]
        ...
        dinv_var = torch.sum(dx_hat * x_mu, dim=0)
        dx_mu1 = dx_hat * inv_var
        dvar = dinv_var * (-0.5) * (var + eps)**(-1.5)
        dx_mu2 = dvar * (2.0 / N) * x_mu
        dx_mu = dx_mu1 + dx_mu2
        dmean = torch.sum(dx_mu * -1.0, dim=0)
        dx1 = dx_mu
        dx2 = dmean * (1.0 / N)
        dx = dx1 + dx2
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W =x.shape
        x_tranposed = x.permute(0,2,3,1)
        x_flat = x_tranposed.reshape(-1, C)
