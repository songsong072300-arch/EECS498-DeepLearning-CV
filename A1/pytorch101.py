...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    while i<=stop :
        if i%10==0 :
            ...
            i+=1
    x=torch.arange(i,stop+1,10,dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    even_rows_odd_cols=x[0:row:2,1:col:2]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    out = (
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    y[torch.arange(row),torch.tensor(x)]=1
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    y=x.clone()
    idx0=y.argmin(dim=1)
    idx1=torch.arange(len(y),device=y.device)
    y[idx1,idx0]=0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    miu=x.sum(dim=0)/x.shape[0]
    std=torch.sqrt(((x-miu)**2).sum(dim=0)/(x.shape[0]-1))
    y=(x-miu)/std
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    pre_sum=torch.cat([torch.zeros(1),torch.cumsum(x,dim=0)],dim=0)
    right_index=ls.cumsum(0)
    num=right_index.shape[0]
    left_index=torch.cat([torch.zeros(1,dtype=right_index.dtype),right_index[0:num-1]],dim=0)
    sum=pre_sum[right_index]-pre_sum[left_index]
    y=sum/ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
